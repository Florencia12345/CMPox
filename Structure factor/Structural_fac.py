...
# calculate the brag diffraction angle
    ...
    def find_f(self, wavelength, list_name):
        wavelength = round(wavelength, 2)
        index1 = np.floor(wavelength * 10) -1
        index2 = (wavelength * 100) % 10 - 1
        # the lists for the Xray coefficients f' and f'' are a 3D list
        listname = eval("self." + list_name)
        f1 = listname[index1][index2][0]
        f2 = listname[index1][index2][1]
